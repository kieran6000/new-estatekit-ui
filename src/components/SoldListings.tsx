  ...
  return (
    <Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1 }}>
          {tiles.map((l) => (
            <Box key={l.id} sx={{ position: "relative", borderRadius: "8px", overflow: "hidden", aspectRatio: "4 / 3", bgcolor: "#e4e7e5" }}>
              <Box component="img" src={l.imageUrl!} alt="Recently sold home" loading="lazy" sx={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
              <Box sx={{ position: "absolute", left: 4, bottom: 4, bgcolor: accent, color: "#fff", fontWeight: 800, fontSize: 10, letterSpacing: "0.5px", borderRadius: "4px", px: 0.75, py: 0.25 }}>SOLD</Box>
            </Box>
          ))}
        </Box>
      )}
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", flexWrap: "wrap", gap: 0.5, mt: 1.25, fontSize: 13, fontWeight: 600, color: "#2b2820" }}>
        ...
        <span>5.0</span>
        <span>·</span>
        <span>{listings.length} recent {listings.length === 1 ? "sale" : "sales"}{area ? ` in ${area}` : ""}</span>
      </Box>
    </Box>
  );
}
...
export function SoldList({ listings }: { listings: SoldListing[] }) {
  ...
  return (
    <Box>
      <Typography sx={{ fontSize: 17, fontWeight: 800, textAlign: "center", mt: 2, mb: 1.5, color: "#17150f" }}>Recent Sales</Typography>
        ...
        {listings.map((l) => (
          <Box key={l.id} sx={{ borderRadius: "12px", overflow: "hidden", bgcolor: "#fff", boxShadow: "0 8px 24px -18px rgba(20,40,30,.4)", border: "1px solid #eceef1" }}>
            ...
            <Box sx={{ p: "12px 14px", textAlign: "center" }}>
              ...
              {l.price != null && <Typography sx={{ fontSize: 16, fontWeight: 800, color: "#17150f" }}>Sold for {rands(l.price)}</Typography>}
            </Box>
          </Box>
        ))}
      </Box>
